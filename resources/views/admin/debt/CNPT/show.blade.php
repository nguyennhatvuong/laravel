         ...
         <div class="collapse panel-collapse show collapse-stock" id="CNPT">
            <div class="card">
               <div class="card-header">
                  <h4 class="m-0 font-weight-bold text-primary float-left">Chi tiết công nợ phải thu</h4>
                  ...
                  <a class="btn btn-primary btn-sm float-right mr-2" href="{{ route('admin.CNPT') }}"> Quay lại</a>
               </div>
               <div class="card-body">
                  <div class="table-responsive">
                     ...
                     <table style="color: #333" class="table  table-hover"   cellspacing="0">
                        <thead>
                            ...
                            <tr>
                                ...
                                <td></td>
                                <td></td>
                                <td style="font-weight:bold">Mã số: {{ $user->code }}</td>
                                <td></td>
                                <td></td>
                                <td></td>
                                <td></td>
...
@push('scripts')
    <script>
        ...
        getDataCNPT(start, end, 'CNPT',user_id);
        ...
        $('#reportrange-PT').on('apply.daterangepicker', (e, picker) => {
            ...
            var start = arr[0];
            var end = arr[1];
            getDataCNPT(start, end, cat,user_id);
        });
          function getDataCNPT(start, end, cat,user_id) {
            $.ajax({
                type: 'post',
                dataType: 'json',
                ...
                success: function(result) {
                  ...
                  if(detail_phatsinh!=null){
                    ...
                    for(var item of detail_phatsinh){
                      var date =moment(item['data']).format("DD/MM/YYYY");
                      content+='<tr>'+
                        '<td>'+stt+'</td>'+
                        '<td>'+item['document']+'</td>'+
                        '<td>'+date+'</td>'+
