...
@section('title', 'CÔNG NỢ PHẢI CHI')
         ...
         <div class="collapse panel-collapse show collapse-stock" id="CNPC">
            <div class="card">
               <div class="card-header">
                  ...
                  <a class="btn btn-primary btn-sm float-right mr-2" href="{{route('admin.CNPT')}}"> Công nợ phải thu</a>
               </div>
               <div class="card-body">
                  <div class="table-responsive">
                     <input class="auto-numeric" type="hidden">
                     <table style="color: #333" class="table table-bordered display" id="table-CNPC"  width="100%" cellspacing="0">
                        <thead>
                            <tr>
                          ...
                          <tfoot>
                            <tr>
                                <th colspan="4" style="font-size:18px; text-align: center;vertical-align: middle;">Tổng cộng</th>
...
@push('scripts')
    <script>
        ...
        //  getDataCNPC(start, end, 'CNPC'); 
        $('#reportrange-CNPC').on('apply.daterangepicker', (e, picker) => {
            var cat = "CNPC";
            var item = $('#reportrange-CNPC span').text();
            var arr = item.split(' - ');
            var start = arr[0];
            var end = arr[1];
            $('#table-CNPC').DataTable().destroy();
            getDataCNPC(start, end, cat);
        });
          function getDataCNPC(start, end, cat) {
            var numberFormat = $.fn.dataTable.render.number( '\,', '.', 0 ).display;

            var table = $('#table-CNPC').DataTable({
                responsive: true,
                dom: 'Blfrtip',
                {{--  dom: 'lfrtip',  --}}
                processing: true,
                ...
                $( api.column( 0 ).footer() ).html('Tổng cộng');
                    $( api.column( 4 ).footer() ).html(numberFormat(no_dau_ky));
                    $( api.column( 5 ).footer() ).html(numberFormat(co_dau_ky));
                    $( api.column( 6 ).footer() ).html(numberFormat(no_phat_sinh));
                    $( api.column( 7 ).footer() ).html(numberFormat(co_phat_sinh));
                    $( api.column( 8 ).footer() ).html(numberFormat(no_cuoi_ky));
                    $( api.column( 9 ).footer() ).html(numberFormat(co_cuoi_ky));
                },
